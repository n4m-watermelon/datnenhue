        ...
        try {
            ...
            if ($fileUpload->getSize() / 1024 > (int)config('media.max_file_size_upload')) {
                if (request()->ajax()) {
                    return [
                        'error' => true,
                        'message' => trans('media.file_too_big',
                            ['size' => config('media.max_file_size_upload')]
                        )
                    ];
                }
                return redirect()->back()->with('error', trans('media.file_too_big'));
            }
            ...
            if (empty($data['mime_type'])) {
                return [
                    'error' => true,
                    'message' => trans('media.can_not_detect_file_type'),
                ];
            }
