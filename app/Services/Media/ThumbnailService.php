...
class ThumbnailService
{
    ...
    public function save($type = 'fit')
    {
        ...
        /*$watermark  = $this->imageManager->make(public_path('upload/settings/' . Setting::getSetting('site')
                ->watermark))->widen
        (floor($thumbImage->width() / 3), function ($constraint) {
            $constraint->upsize();
        });
        $thumbImage->insert($watermark, 'bottom-right');*/
